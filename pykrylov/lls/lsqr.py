# -*- coding: utf-8 -*-
"""Linear least-squares solver: LSQR.

This is a line-by-line translation from Matlab code available at
http://www.stanford.edu/~saunders/lsqr with several Pythonic enhancements.
...
class LSQRFramework(KrylovMethod):
    ...
    def __init__(self, A, **kwargs):
        u"""Instantiate a LSQR solver for `Ax = b`.

        LSQR solves  `Ax = b` or `minimize ‖b - Ax‖ + damp * ‖x‖` in Euclidian
        norm.

        :parameters:
            :A:     A (m×n) linear operator defined by  `y = A * x`
                    (or `y = A(x)`), where `y` is the result of applying the
                    linear operator to `x`. Application of transpose linear
                    operator must be accessible via `u = A.T * x`
                    (or `u = A.T(x)`). The shape of the linear operator `A`
                    must be accessible via `A.shape`. A convenient way to
                    achieve this is to make sure that `A` is a
                    :class:`LinearOperator` instance.
        """
        # Initialize.
    ...
    def solve(self, rhs, itnlim=0, damp=0.0, M=None, N=None,
        ...
        u"""Solve the regularized linear least-squares problem.

        Solve the linear system, linear least-squares problem or regularized
        linear least-squares problem with specified parameters:
        ...
        is specified)
            ...
            `minimize ‖b - Ax‖ + damp * ‖x‖`
                s.t.  ‖x‖ ⩽ Δ

        :warning: a trust-region constraint only applies in the Euclidian norm.
                  It is incompatible with setting preconditioners `N` and `M`.

        All return values below are stored in members of the same name.
        ...
        :parameters:
           ...
           :N:      a linear operator if nonzero.

        :keywords:

           :atol:
           :btol:  are stopping tolerances.  If both are 1.0e-9 (say),
                   the final residual norm should be accurate to about 9
                   digits.
           ...
           :window: Number of consecutive iterations over which the direct
                    error should be measured (default: 5).
        ...
        :return:
           ...
           :r1norm: = ‖r‖, where r = b - Ax.
           :r2norm: = sqrt(‖r‖² + damp² ‖x‖^2)
                    = r1norm if damp = 0.
           :Anorm: = estimate of Frobenius norm of (regularized) A.
           :Acond: = estimate of cond(Abar).
           :Arnorm: = estimate of ‖Aᵀr - damp² x‖.
           :xnorm: = norm(x).
           :var:   (if present) estimates all diagonals of (AᵀA)^{-1}
                   (if damp=0) or more generally (AᵀA + damp²I)^{-1}.
        ...
        while itn < itnlim and not x_is_zero:
            ...
            if radius is not None:
                ...
                roots = roots_quadratic(
                    ww, 2 * xw, xnorm * xnorm - radius * radius)
        ...
        if istop in [3, 6]:
            self.status = 'ill-conditioned operator'
        if istop == 7:
