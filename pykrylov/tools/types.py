...
real_types = [np.float, np.float_, np.float16, np.float32, np.float64]
if hasattr(np, 'float128'):
    real_types += [np.float128]

complex_types = [np.complex, np.complex_, np.complex64, np.complex128]
if hasattr(np, 'complex256'):
    complex_types += [np.complex256]
